...
type RestOf<T extends ITransaction | NewTransaction> = Omit<T, 'transactionDate' | 'createdAt' | 'updatedAt'> & {
...
export class TransactionService {
  ...
  protected convertDateFromClient<T extends ITransaction | NewTransaction | PartialUpdateTransaction>(transaction: T): RestOf<T> {
    return {
      ...
      updatedAt: transaction.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransaction: RestTransaction): ITransaction {
    return {
      ...
      updatedAt: restTransaction.updatedAt ? dayjs(restTransaction.updatedAt) : undefined,
    };
  }
