...
type RestOf<T extends IMember | NewMember> = Omit<T, 'createdAt'> & {
...
export class MemberService {
  ...
  protected convertDateFromClient<T extends IMember | NewMember | PartialUpdateMember>(member: T): RestOf<T> {
    return {
      ...member,
      createdAt: member.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMember: RestMember): IMember {
    return {
      ...restMember,
      createdAt: restMember.createdAt ? dayjs(restMember.createdAt) : undefined,
    };
  }
